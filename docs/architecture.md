...
- `warnings_json` is a JSON array of codes from the set `TOTAL_MISMATCH`, `MISSING_DATE`, `MISSING_STORE`, `FUTURE_DATE`, `UNMATCHED_LINES`, `MATCHING_FAILED`, `POSSIBLE_DUPLICATE`.
  ...
  The pointer is the warning: an API response leaves `POSSIBLE_DUPLICATE` out when `possible_duplicate_of` is null, because the column is `ON DELETE SET NULL` and deleting the other receipt — what the household does when the warning is right — would otherwise leave a warning naming nothing.
  `DELETE /api/receipts/:id` also clears the code from the rows that pointed at the deleted receipt, so the stored state matches what is served (T41).
...
- `products.parent_id` groups a product's variants under it (T40, ADR-0019), depth exactly one: a product with a parent cannot itself be a parent, and a product with children cannot get a parent, both enforced in the route handlers.
  ...
  `foldHistories` (`src/server/domain/productGroups.ts`) sums children's purchases into the parent's `ProductHistory` before the suggestion engine, refresh, dismissals or the AI proposal context see it; a receipt, its lines, aliases and `ProductDetail.purchases` always keep the variant.
  A parent's summary stats (`timesBought`, `lastBought`, `medianIntervalDays` on `Product`, in the products list and `topProducts`) are the group's folded total, not the parent's own lines, so the group ranks by its combined habit instead of under its own variants and a habit is not counted twice; a child keeps its own summary stats (T40 review F2).
...
- A size guard: a serialised context over 120,000 characters walks a narrowing ladder — the 12-week scope, then 250 products by most recent purchase (ties by id), then 6, 3 and 1 purchases per product, then 150 and 75 products — and stops at the first rung that brings it under, logging one `info` line with the rung, the guard and the serialised length before and after. If no rung brings it under, the context is sent anyway, because a proposal that costs more is better than no proposal, and that is the only case that logs a `warn`. The 120,000 is provisional: it is roughly 30k prompt tokens at four characters per token, a prose ratio applied to JSON full of date strings, so the real figure may be nearer 55k; the logged length and the call's own `promptTokens` settle it on the first real proposal (T45, issue #15).
...
type Product = {
  ...
  // For a parent (variantCount > 0), these three are the group's folded stats, not the parent's
  // own lines; a child (parentId set) reports its own, same as an ungrouped product (T40 review F2).
...
| `GET /api/receipts?limit=50&offset=0` | — | `200 ReceiptSummary[]` | Ordered by the receipt's own date, newest first: `purchased_at`, falling back to the upload date while the receipt has none, with `id` breaking ties. Offset pagination, because the id no longer says where the next page starts. |
...
| `DELETE /api/receipts/:id` | — | `204` | Cascades lines and image. Removes `POSSIBLE_DUPLICATE` from the `warnings_json` of every receipt pointing at this one, in the same transaction and before the delete, since `ON DELETE SET NULL` would otherwise make those rows unfindable (T41). |
...
| `GET /api/products?q=&includeSuppressed=` | — | `200 Product[]` | `q` filters on `name_normalized` contains `normalizeText(q)`. Ordered by `lastBought` desc, a product never bought last, ties by name; a child sorts directly after its parent when both are in the result (T40). |
| `GET /api/products/:id` | — | `200 ProductDetail` | `parent`/`variants`/`groupStats` T40; `groupStats` is `null` unless the product is a parent. Unlike the products list and `topProducts`, `ProductDetail`'s own top-level `timesBought`/`lastBought`/`medianIntervalDays` stay the product's own lines even for a parent — `groupStats` carries the folded total instead, so a parent's page can show both (T40 review F2). |
...
| `/scan` | ScanPage | "Ta bilde" (`<input type="file" accept="image/*" capture="environment">`, one photo) and "Velg fra bilder" (same input without `capture`, `multiple`). Images can also be dropped anywhere on the page: the drop carries the same `File` objects the input gives, so they enter the same queue and there is no second upload path, and a drop containing anything that is not an image uploads only the images and says `Bare bilder kan lastes opp` once. While a file is over the window the page shows a full-screen `Slipp bildene her`, which is in the DOM only while the drag lasts (T42). An image on the clipboard is uploaded by pasting it on the page, through the same queue and with the same rule about non-images; a pasted file with no name shows as `Limt inn bilde`. Pasting works where the browser delivers a clipboard image to the page — desktop browsers, and an iPad with a keyboard; on an iPhone the route is the share sheet, not the clipboard (T44). Every selected file is downscaled and uploaded at once, one after the other in selection order, in a list with per-file state: "Laster opp … 45 %", "Lastet opp", "Allerede skannet" with a link to the existing receipt, or "Feilet: {message}". Below the list, "Skann (1)" or "Skann alle (n)" for every receipt with status `uploaded`; it calls scan for each and navigates to `/receipts/:id` when n is 1, else to `/receipts`. |
...
| `/products` | ProductsPage | Search field, list ordered by last bought (newest first, never-bought last, ties by name) with times bought, last bought, interval; toggle to show suppressed; a parent additionally shows `n varianter` under its name, and its times-bought line is the group's folded total (T40 review F2); a child renders indented under its parent when both are in the result, otherwise with a `variant av …` line under its name; a collapsed `Kan være samme vare (n)` block above the list when `GET /api/products/group-candidates` has candidates, each row naming its members and a suggested group name (editable) with `Grupper` to create the group and `Ikke nå` to hide it (remembered per candidate in `localStorage`, no server state). T40. |
| `/products/:id` | ProductPage | Rename, category select, "Ikke foreslå" toggle, merge into another product (confirm text now also says `Er det varianter av samme vare, bruk Varegruppe i stedet.`), aliases with delete, purchase history. A `Varegruppe` section between "Ikke foreslå" and "Alias": ungrouped shows `Legg i gruppe…`, a page-local search-and-pick component (`GroupPicker`, styled like the existing `MergeSearch` on this page rather than reusing `ProductPicker`, which is tied to receipt-line matching) — choosing a parent attaches directly; choosing another ungrouped product opens a `Navn på varegruppen` dialog and calls `POST /api/product-groups`; typing a name with no match offers `Opprett «…»`, creating a new parent with this product as its only member. A variant shows `Variant av «<parent>»` as a link and `Fjern fra gruppen`; a parent shows `Varianter (n)` as links with each variant's own count and last-bought date, and the group's folded `groupStats` line, with no `Legg i gruppe`. T40. |
...
- Checking an item on the shopping list is optimistic; failure reverts and shows a toast.
  Removing an item is deferred the same way: the row hides at once and a 6 s `Angre` toast can bring it back with nothing sent; otherwise the delete goes out when the toast expires, a second removal starts, or the list view unmounts, so an item removed just before the app is closed simply stays on the list (T31).
