...
Creating the service and setting `APP_PASSWORD`, `SESSION_SECRET`, `MOONSHOT_API_KEY`, `XAI_API_KEY` and the four `LITESTREAM_*` secrets is a manual step in the Render dashboard; the blueprint marks them `sync: false` for exactly that reason. Only the active `LLM_PROVIDER`'s key needs a real value (ADR-0015).
